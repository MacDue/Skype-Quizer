// Borrowed from https://github.com/yourWaifu/sleepy-discord
...
namespace SleepyDiscord {
	namespace json {
		...
		inline std::string_view toStdStringView(const Value& value) {
		...
		template<>
		struct ClassTypeHelper<std::string_view> : public EmptyFunction<std::string_view> {
			static inline std::string_view toType(const Value& value) {
				return toStdStringView(value);
			}
			static inline Value fromType(const std::string_view& value, Value::AllocatorType&) {
...
#define JSONStructEnd
		...
		template<class ResultingObject>
		inline ResultingObject fromJSON(const std::string_view& json) {
